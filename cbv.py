...
def _cbv(router: APIRouter, cls: Type[T], get_self) -> Type[T]:
    """
    ...
    for route in cbv_routes:
        ...
        _update_cbv_route_endpoint_signature(cls, route, get_self)
...
def _update_cbv_route_endpoint_signature(cls: Type[Any], route: Union[Route, WebSocketRoute], get_self) -> None:
    """
    ...
    new_first_parameter = old_first_parameter.replace(default=Depends(get_self))
    new_parameters = [new_first_parameter] + [
        parameter.replace(kind=inspect.Parameter.KEYWORD_ONLY) for parameter in old_parameters[1:]
    ]
    # new_parameters = [new_first_parameter] + [
    #     parameter for parameter in old_parameters[1:]
    # ]
    new_signature = old_signature.replace(parameters=new_parameters)
